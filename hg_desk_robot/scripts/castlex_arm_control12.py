        ...
        while(1):
            	...
            	if self.id == 1:
                    if self.count == 0:
                        ...
                        # 像素坐标系相对于机械臂坐标系变换
                        x = 85*(self.y - 239.5)/self.side - 75  #60
                        y = -85*(self.x - 319.5)/self.side + 17.5  #17.5
                        z = -162.5 #162

                    elif self.count == 1:

                        # 像素坐标系相对于机械臂坐标系变换
                        x = 85*(self.y - 239.5)/self.side - 70
                        y = -85*(self.x - 319.5)/self.side + 17.5
                        z = -162.5

                    time.sleep(1)
                    if self.count == 0:
                        self.HG_DR_C.moveAnInterval(x, y, z+30)
                        time.sleep(1)
                    ...
                    # 第一次夾取後復位
                    if self.count == 1:
                        self.HG_DR_C.moveAnInterval(-135, 10, -125)
                        time.sleep(1)
                        self.HG_DR_C.calibrateJoint()
                        self.HG_DR_C.moveAnInterval(2, -60, 130)
                        time.sleep(1)
