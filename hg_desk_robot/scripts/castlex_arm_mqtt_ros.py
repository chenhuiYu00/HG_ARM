...
class arm():
    def __init__(self):
        ...
        self.on_subscribe()
        #   订阅语音识别的话题
        self.obiect_color_pub = rospy.Publisher("voice/object_color", Int64, queue_size=1)

        self.action, self.control_state_data = False, 0
        self.castlex, self.arm = False, False
        self.sub_name, self.sub_dir, self.sub_action, self.sub_feedback, self.sub_msg, self.object_msg = None, None, None, None, None, None
        #   初始化ros节点
    ...
    def mqtt_data(self, pose):
        ...
        if self.action:
            ...
            if self.control_state_data == 0:
                self.obiect_color_pub.publish(self.object_msg[0])
                time.sleep(5)
            ...
            #   第二次识别
                ...
                self.obiect_color_pub.publish(self.object_msg[1])
                time.sleep(5)
                self.robotInitDone_pub.publish(8)
                time.sleep(5)
                self.control_pub.publish(1)
                ...
                self.control_state_data = -1
            #   完成二次识别
            elif self.control_state_data == 2:
                print("done")
                self.action = False
                self.arm = True
	...
	# python3 bytes转str
        ...
        elif len(self.sub_msg) == 6:
            self.sub_name, self.sub_dir, self.sub_action, self.sub_feedback, self.object_msg = self.sub_msg['name'], self.sub_msg['dir'], self.sub_msg['ation'], self.sub_msg['feedback'], self.sub_msg['object']
        else:
            pass
